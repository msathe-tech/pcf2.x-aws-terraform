resource "aws_s3_bucket" "buildpacks_bucket" {
  bucket        = "${var.env_name}-ilab-buildpacks-bucket"
  force_destroy = true
...
resource "aws_s3_bucket" "droplets_bucket" {
  bucket        = "${var.env_name}-ilab-droplets-bucket"
  force_destroy = true
...
resource "aws_s3_bucket" "packages_bucket" {
  bucket        = "${var.env_name}-ilab-packages-bucket"
  force_destroy = true
...
resource "aws_s3_bucket" "resources_bucket" {
  bucket        = "${var.env_name}-ilab-resources-bucket"
  force_destroy = true
